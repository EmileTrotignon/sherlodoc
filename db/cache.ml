...
(** Application of the [Make] functor. They are not at toplevel because we can't
    shadow modules.*)
  ...
  module Map (A : Memo) (M : Map.S) = Make (struct
    ...
    let sub ~memo m =
      ...
      | M.Node { l; v; d; r; h } ->
          ...
          let uid_r, r = memo r in
          ( Node { l = uid_l; v; d = uid_d; r = uid_r; h }
          , M.Node { l; v; d; r; h } )
  end)
...
(* Hiding the uids *)
module String = Strip (String)
module Char_list = Strip (Char_list)
module String_list = Strip (String_list)
module String_list_list = Strip (String_list_list)
module Kind = Strip (Kind)
module Elt_array = Strip (Elt_array)
module Elt_array_occ = Strip (Elt_array_occ)
